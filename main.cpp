...
struct Carrier {
    ...
    Carrier(int _id, CarrierType _ct, int _capacity=0, int _keeping=0, int _used=0)
        :id(_id), ct(_ct), capacity(_capacity), keeping(_keeping), used(_used) {}

    string toString() const& {
        return string(1, ct) + to_string(id) + ":" + to_string(keeping) + ":" + to_string(used);
    }
};
...
struct State {
    ...
    State()
        :depth(0) {}
    State(vector<Carrier> const& _cars, vector<CircuitBreaker> const& _cbs, int _depth=0)
